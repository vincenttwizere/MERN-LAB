...
export default function EditMovie() {
  ...
  useEffect(() => {
    const demoMovie = { title: `Movie ${id}`, year: '2025', genre: 'Action', posterUrl: '' }
    setForm(demoMovie)
    setLoading(false)
  }, [id])
  ...
  return (
    <div className="px-6 pb-20 lg:px-10">
      <div className="mb-8">
        ...
        <p className="text-sm text-neutral-400">Update movie details in the CineWatch database.</p>
      </div>
      <form onSubmit={submit} className="grid gap-6 rounded-[32px] border border-white/10 bg-[#09090f] p-8 shadow-glow">
        ...
        <button disabled={saving} className="w-full rounded-full bg-brand px-6 py-4 text-sm font-semibold text-white transition hover:bg-red-600">{saving ? 'Saving...' : 'Update Movie'}</button>
      </form>
    </div>
  )
}
