...
export default function Profile(){
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      try{
        ...
        if (mounted) setList(Array.isArray(res) ? res : [])
  ...
  const stats = list.reduce((acc, item)=>{
    ...
    if (item.rating) { acc.sumRating += item.rating; acc.countRating++ }
    return acc
    ...
    <div className="p-6 max-w-3xl mx-auto">
      <div className="bg-neutral-800 p-6 rounded">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-full bg-neutral-700 flex items-center justify-center text-xl">{user?.username ? user.username.charAt(0).toUpperCase() : 'U'}</div>
          <div>
